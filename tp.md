...
Roles sugeridos para el módulo:
- **Admin**: administración completa de usuarios, roles y permisos.
- **Vendedor**: gestión diaria de catálogo y seguimiento de compras.
- **Visitador**: monitoreo en solo lectura del estado de ventas.

Validación personal: corro `node scripts/check-login-roles.js` para pegarle directamente a `/api/auth/login` con los tres usuarios de prueba y revisar el `role` que sale del backend. También probé contra la demo en Vercel para dejar captura de que `admin`, `vendedor1` y `visitador1` reciben justamente `admin`, `vendedor` y `visitador`. Esto cierra la consigna de “Integración con sistema de permisos, control de permisos y gestión de errores” porque no dependo de mocks ni del frontend para verificarlo.
